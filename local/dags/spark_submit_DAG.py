...
SPARK_SCRIPT_KEY = Variable.get("SPARK_SCRIPT_KEY")
...
with DAG(
	...
	send_bootstrap_s3 = PythonOperator(
		task_id="send_bootstrap_s3",
		python_callable=upload_to_s3,
		op_kwargs={
			"filename": "emr_bootstrap/emr_bootstrap.sh",
			"s3_key": BOOTSTRAP_KEY 
		}
	)


	send_spark_script_s3 = PythonOperator(
		task_id="send_spark_script_s3",
    	python_callable=upload_to_s3,              # function to be executed
		op_kwargs={
			"filename": "src/",
			"s3_key": SPARK_SCRIPT_KEY 
		}
	)
	...
	start_data_pipeline >> [send_bootstrap_s3, send_spark_script_s3] >> create_emr_cluster >> add_emr_steps 
